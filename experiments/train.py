...
from multiagentnfv.nfvparser import Parser as nParser
...
def get_trainers(env, obs_shape_n, arglist, features, edge_features):
    ...
    for i in range(env.n):
        ...
        trainers.append(ReplayBuffer(6e4))
    return trainers
...
def process_request(request_n):
    ...
    for request in request_n:
        ...
        prsr = nParser(request)
        prsr.parse()
        reqPlacementInput = prsr.create_pairs()
        combined_dict_n.append(deepcopy(reqPlacementInput))
    ...
    gc.collect()
    return train_vnf_n, tran_pos_n, combined_dict_n
...
def train(arglist):
    with U.multi_threaded_session():  # 只使用一个cpu的会话
        ...
        train_vnf_n, tran_pos_n, combined_dict_n = process_request(request_n)
        ...
        while True:
            ...
            action_n = []
            network = dict()
            network['nodes'] = list(env.world.topo.G.nodes)
            network['edges'] = list(env.world.topo.G.edges)
            for edge in network['edges'][:]:
                if edge[0] != edge[1]:
                    network['edges'].append((edge[1], edge[0]))
            network['nodeCap'] = {node: env.world.topo.G.nodes[node]['nodeCap'] for node in env.world.topo.G.nodes}
            network['nodeMem'] = {node: env.world.topo.G.nodes[node]['nodeMem'] for node in env.world.topo.G.nodes}
            network['edgeDatarate'] = {edge: env.world.topo.G.edges[edge]['edgeDatarate'] for edge in env.world.topo.G.edges}
            for edge in network['edgeDatarate'].copy():
                if edge[0] != edge[1]:
                    network['edgeDatarate'][(edge[1], edge[0])] = network['edgeDatarate'][edge]
            network['edgeLatency'] = {edge: env.world.topo.line_G.nodes[edge]['edgeLatency'] for edge in env.world.topo.G.edges}
            for edge in network['edgeLatency'].copy():
                if edge[0] != edge[1]:
                    network['edgeLatency'][(edge[1], edge[0])] = network['edgeLatency'][edge]
            network['F'] = ['CACHE', 'LB', 'FW', 'WAPGW', 'IDS', 'PCTL', 'IPS', 'VOPT', 'HHE', 'AV', 'WOPT', 'PRX', 'DPI']
            network['c_req'] = {'FW': 0.55, 'CACHE': 0.3, 'DPI': 0.8, 'PCTL': 2.0, 'WAPGW': 0.5, 'VOPT': 0.7, 'HHE': 0.5, 'IDS': 0.6,
                                'LB': 0.3, 'BNG': 0, 'CR': 0, 'GGSN': 0, 'WWW': 0, 'REG': 0, 'SRV': 0, 'PRX': 0.5, 'AV': 0.7, 'WOPT': 0.6, 'IPS': 0.6}
            # network['m_req'] = {'FW':0.55,'CACHE':0.3,'DPI':0.8,'PCTL':2.0,'WAPGW':0.5,'VOPT':0.7,'HHE':0.5,'IDS':0.6,'LB':0.3,'BNG':0,'CR':0,'GGSN':0,'WWW':0,'REG':0,'SRV':0,'PRX':0.5,'AV':0.7,'WOPT':0.6,'IPS':0.6}
            # network['process_time'] = {'FW': 0.05, 'CACHE': 0.03, 'DPI': 0.08, 'PCTL': 0.02, 'WAPGW': 0.05, 'VOPT': 0.07, 'HHE': 0.05, 'IDS': 0.06, 'LB':
            #                            0.03, 'BNG': 0, 'CR': 0, 'GGSN': 0, 'WWW': 0, 'REG': 0, 'SRV': 0, 'PRX': 0.05, 'AV': 0.07, 'WOPT': 0.06, 'IPS': 0.06}
            encoder = env.world.topo.usage_encoder
            successful_n = []
            # g = pickle.load(open('nfvmaddpg/model/gan/data/netGraph_abilene.pickle', "rb"))
            for agent_index, agent in enumerate(env.agents):
                placementInput = PlacementInput(dict(combined_dict_n[agent_index]))
                # result = Placement(dict(g), placementInput).solve()
                result = Placement(network, placementInput).solve()
                # action = [np.zeros((1, 12, 12)), np.zeros((1, 12, 12))]
                successful_n.append(result['solved'])
                if result['solved'] == False:
                    action_n.append([np.zeros((1,12,12)), np.zeros((1,12,12))])
                else:
                    node_with_usage = np.zeros((12, 12))
                    for usage, nodes in result['ureq_to_node'].items():
                        usage_name = usage.split('_')[1]
                        if usage_name.startswith('a'):
                            continue
                        usage_index = encoder[usage_name]
                        for node in nodes:
                            node_with_usage[node, usage_index] = 1
                    for row_index, row in enumerate(node_with_usage[: , :]):
                        if np.all(row==0):
                            node_with_usage[row_index][0] = -1
                    links_usage = np.zeros((12, 12))
                    for edge, links in result['edges'].items():
                        # start_vnf = edge[0].split('_')[1]
                        # end_vnf = edge[1].split('_')[1]
                        # start_index = encoder[start_vnf]
                        # end_index = encoder[end_vnf]
                        for link in links:
                            links_usage[link[0], link[1]] = 1
                    action_n.append([np.expand_dims(node_with_usage, 0), np.expand_dims(links_usage, 0)])
                    # usage_name = '_'.join(usage_pair[0].split('_')[1:])
            ...
            # collect experience
            # prev = psutil.Process(os.getpid()).memory_info().rss
            for i, buffer in enumerate(trainers):
                ...
                if successful_n[i] == True:
                    buffer.add(obs_n[i], action_n[i], rew_n[i], new_obs_n[i], done_n[i])
                # agent.experience(obs_n[i], action_n[i], rew_n[i], new_obs_n[i], done_n[i], terminal)
            ...
            if len(episode_rewards) > arglist.num_episodes:
                for agent_index, buffer in enumerate(trainers):
                    file_name = 'buffer_pkl/buffer_%d.pkl' % agent_index
                    with open(file_name, 'wb') as fp:
