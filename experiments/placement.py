...
class Placement:
	def __init__(self, network, data, objective_mode='lex', bounds={}):
		...
		self.V = self.G['nodes']
		# C: list of network graph node cpu
		self.C = self.G['nodeCap']
		# C: list of network graph node mem
		# self.Mem = self.G['nodeMem']
		...
		for u in self.data.U:
			...
			# self.FINAL_M_REQ[u] = self.data.in_rate[u] * self.M_REQ[self.data.UF[u]]
		# print ("TOTALC_REQ", self.FINAL_C_REQ)
		# # print("TOTALM_REQ", self.FINAL_M_REQ)
		...
		for v in self.V:
			...
			# self.util_m[v] = self.model.addVar(vtype = GRB.CONTINUOUS, lb = 0, ub = 1, name = "util_m_%s" % (v))
			self.remaining_node_cap[v] = self.model.addVar(vtype = GRB.CONTINUOUS, name = "remaining_node_cap_%s" % (v))
			# self.remaining_node_mem[v] = self.model.addVar(vtype = GRB.CONTINUOUS, name = "remaining_node_mem_%s" % (v))
		"""
		...
		for v in self.V:
			...
			# self.model.addConstr(quicksum(self.m[u,v] * self.FINAL_M_REQ[u] for u in self.data.U), GRB.LESS_EQUAL, self.Mem[v])
		...
		for v in self.V:
			...
			# self.model.addConstr(quicksum(self.m[u,v] * self.FINAL_M_REQ[u] for u in self.data.U) * (1 / self.Mem[v]), GRB.EQUAL, self.util_m[v])
			#~ elif self.c_s[v] <> 0:
		...
		for v in self.V:
			...
			# self.model.addConstr(self.C[v] - quicksum(self.m[u,v] * self.FINAL_M_REQ[u] for u in self.data.U), GRB.EQUAL, self.remaining_node_mem[v])
